...
// time, and the owners facet each page carries. The facet rides the same NodeListResponse shape a folder listing
// uses, so a page renders owner attribution the identical way, scoped to that page's own hits rather than a whole
// folder's. A blank query never reaches the RA -- the page calls load only once it has a non-empty term -- so there
// is no listing-vs-no-query ambiguity to track here.
//----------------------------------------------------------------------------------------------------------------------

import { computed, ref } from 'vue';
import { defineStore } from 'pinia';

import { DEFAULT_SEARCH_LIMIT, type NodeResponse, type UserSummary } from '@fileshed/core';
...
export const useSearchStore = defineStore('search', () =>
{
    ...
    return { q, nodes, owners, total, loading, loadingMore, error, hasMore, isEmpty, load, loadMore, retry, clear };
});
