...
import type { AdminUserResponse, MeResponse } from '@fileshed/core';
...
describe('UserRow', () =>
{
    ...
    {
        const uncapped = mountRow(user({ usedBytes: 4096, quotaLimit: null }));
        expect(uncapped.text()).not.toContain('%');

        const comfortable = mountRow(user({ usedBytes: 4500, quotaLimit: 10_000 }));
        expect(comfortable.text()).toContain('(45%)');
        expect(comfortable.find('.text-error').exists()).toBe(false);

        const nearing = mountRow(user({ usedBytes: 8500, quotaLimit: 10_000 }));
